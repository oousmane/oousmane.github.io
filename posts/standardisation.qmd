...
Il n'est pas rare de rencontrer également le terme "normalisation" que l'on emploi souvent à la place bien qu'à la base c'est des concepts très proches, en ce sens qu'il s'agit tout deux de méthodes de transformation de données.
Sans plus tarder, entrons dans le vif du sujet.

## La Normalisation 

Il s'agit d'une technique de mise à l'échelle ou de transformation, dans laquelle chaque point de données est mis à l'échelle de manière à ce qu'ils se situent tous entre 0 et 1.
Cette technique est également connue sous le nom "**transformation min-max"**.
...
On verra par la suite en quoi celà peut être très utile en analyse de données.
...
C'est la technique de transformation de données la plus courante en analyse de données car constituant un préalable à beaucoup d'algorithme de Machine Learning.
...
Maintenant que nous avons une idée assez claire sur les concepts de normalisation et de standardisation, en quoi celà pourrait être utile ?
...
Une fois transformée, on donne le même poids dans un modèle de machine learning poids en g et l'âge en années !
...
Les valeurs de z-score supérieures à 3 (ou inférieures à -3) sont généralement considérées comme des valeurs extrèmes ou douteuses.
...
Il n'existe pas de fonction de base pour calculer ce score, on écrit vite fait nous même notre petite fonction pour celà.
...
Si vous voulez vous pouvez écrire une fonction vite fait, elle n'est pas très compliquée.
...
Et c'est tout !
Mais par principe, c'est mon principe pourquoi réécrire des fonctions qui existent?
